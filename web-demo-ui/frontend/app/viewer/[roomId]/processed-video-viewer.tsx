...
export default function ProcessedVideoViewer() {
  ...
  useEffect(() => {
    ...
    const setupEventHandlers = () => {
      ...
      sfuSocketRef.current.on('host-disconnected', () => {
        ...
        setStats(prev => ({ ...prev, hostStreaming: false, receivingVideo: false, receivingAudio: false }))
    ...
    const displayProcessedVideoFrame = (frameDataUrl: string, boundingBoxes: number[][]) => {
      ...
      try {
        ...
        img.onload = () => {
          ...
          if (boundingBoxes.length > 0) {
            ...
            ctx.fillStyle = 'black'
            ctx.font = '16px Arial'
            ctx.fillText(`🛡️ Privacy Protected: ${boundingBoxes.length} regions`, 15, 30)
          }
          ...
          if (!canvasStreamRef.current) {
            ...
            console.log('[VIEWER] ✅ Canvas stream initialized for processed video')
          }
        }
        img.src = frameDataUrl
        
      } catch (error) {
        console.error('[VIEWER] ❌ Error displaying processed video frame:', error)
      }
    }
  ...
  const getConnectionStatusText = () => {
    switch (connectionState) {
      case 'connected': 
        return stats.hostStreaming ? 'Watching Processed Stream' : 'Connected - Waiting for Stream'
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="bg-white rounded-lg p-6 mb-6">
          ...
          <div className="text-sm text-gray-600 text-center">
            <p><strong>Flow:</strong> Host → Real-time Processing → Privacy Protection → You</p>
